...
const CashFlowManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(`Transaction ${modalType === 'edit' ? 'updated' : 'created'} successfully`);
        ...
        logActivity(
          ...
          `${modalType === 'edit' ? 'Updated' : 'Created'} ${formData.transaction_type} transaction: ${formData.description}`
        );
      } else {
        setError(result.error || `Failed to ${modalType === 'edit' ? 'update' : 'create'} transaction`);
      }
    } catch (error) {
      console.error(`Error ${modalType === 'edit' ? 'updating' : 'creating'} transaction:`, error);
  ...
  if (!hasPermission('read_cash_flow')) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-gray-600 mt-2">You do not have permission to view cash flow data.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">💰 Cash Flow Management</h1>
          <p className="text-gray-600 mt-1">Monitor income, expenses, and account balances</p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{summary.summary.income_count} transactions</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">{summary.summary.expense_count} transactions</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center">
              <div className={`p-3 rounded-full mr-4 ${
                ...
                <span className="text-2xl">{summary.summary.net_income >= 0 ? '📈' : '📉'}</span>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-600">Net Income</p>
                <p className={`text-xl font-bold ${
                  summary.summary.net_income >= 0 ? 'text-blue-600' : 'text-orange-600'
                }`}>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-purple-600">
                  {formatCurrency(
                    summary.account_balances.reduce((sum, acc) => sum + acc.balance, 0)
                  )}
                </p>
                <p className="text-xs text-gray-500">{summary.account_balances.length} accounts</p>
              </div>
            </div>
          </div>
        </div>
                      ...
                      <tr key={transaction.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          ...
                          {transaction.school_name && (
                            <div className="text-xs text-gray-500">📍 {transaction.school_name}</div>
                          )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
      ...
      <Modal
        ...
        title={
          modalType === 'create' ? 'Add New Transaction' :
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Transaction Evidence * {modalType === 'create' && <span className="text-red-500 text-xs">(Required)</span>}
            </label>
            {modalType !== 'view' && (
                ...
                {formData.evidence_file && (
                  <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-md">
                    <p className="text-sm text-green-600">
                      📄 File selected: {formData.evidence_file.name} ({(formData.evidence_file.size / 1024 / 1024).toFixed(2)} MB)
                    </p>
                    {formData.evidence_file.type.startsWith('image/') && (
                      <div className="mt-2">
                        <img 
                          src={URL.createObjectURL(formData.evidence_file)} 
                          alt="Preview" 
          ...
          {modalType !== 'view' && (
            <div className="flex space-x-3 pt-4">
              ...
              >
                {loading ? 'Saving...' : modalType === 'create' ? 'Create Transaction' : 'Update Transaction'}
              </button>
              <button
                type="button"
          ...
          {transactionToDelete && (
            <div className="bg-gray-50 rounded-lg p-4 border">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                <p><strong>Date:</strong> {formatDate(transactionToDelete.transaction_date)}</p>
                <p><strong>Description:</strong> {transactionToDelete.description}</p>
                <p><strong>Type:</strong> {transactionToDelete.transaction_type === 'income' ? '💰 Income' : '💸 Expense'}</p>
